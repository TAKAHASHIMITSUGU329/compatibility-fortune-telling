...
def calculate(person_a, person_b):
    ...
    if score_100 >= 80:
        advice = f"五格分析（天格・人格・地格・外格・総格）の結果、人格{grids_a['jinkaku']}画（{g_a}）と人格{grids_b['jinkaku']}画（{g_b}）は{gogyou_text}であり、非常に良い相性です。姓名判断では五行の相生関係にある画数同士が互いの運気を高め合うとされ、合計{combined_strokes}画も吉数を示しています。この名前の持つ調和的なエネルギーを信頼し、互いの五格が示す強みを活かした関係を築いていきましょう。"
    elif score_100 >= 60:
        advice = f"五格分析において、{name_a}の人格{grids_a['jinkaku']}画（{g_a}）と{name_b}の人格{grids_b['jinkaku']}画（{g_b}）の五行関係は{gogyou_text}で、良好な相性です。姓名判断の五格理論では、人格（主運）同士の五行バランスが対人関係の根幹を司るとされます。合計{combined_strokes}画の示す運勢を意識しつつ、五行の調和を日常の中で補い合う姿勢がさらなる発展につながります。"
    else:
        advice = f"五格分析では、人格{grids_a['jinkaku']}画（{g_a}）と人格{grids_b['jinkaku']}画（{g_b}）が{gogyou_text}となり、画数上は課題のある組み合わせです。しかし姓名判断の五行相剋は「切磋琢磨」の関係とも解釈でき、合計{combined_strokes}画が示す試練は互いを鍛える力にもなります。画数の吉凶に過度にとらわれず、五格が示す各自の強み（天格の家系力、地格の基礎力）を活かして補完し合うことが大切です。"
