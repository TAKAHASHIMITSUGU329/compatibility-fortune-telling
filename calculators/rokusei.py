...
def calculate(person_a: dict, person_b: dict) -> dict:
    ...
    # アドバイス
    advice_parts = [compat_desc + "。六星占術では運命星が示す本質的な性質と12年周期の運命周期の組み合わせから、二人の相性の深層を読み解きます。運命星同士のエネルギーの相互作用を理解し、周期の波を味方につけることで関係をより良い方向に導けるでしょう。"]
    if is_dai_a or is_dai_b:
        advice_parts.append("大殺界は六星占術における12年運命周期のうち陰影・停止・減退の3年間にわたる停滞期であり、運命星のエネルギーが最も弱まる時期です。この期間中は大きな決断や新規事業を控え、内面の充実と既存の関係性の維持・強化に専念しましょう。大殺界が明ければ種子期から新たな運気の上昇が始まるため、その時に備えて二人の絆の土台を固めておくことが肝要です。")
    if pol_a != pol_b:
        advice_parts.append("陰陽が異なる組み合わせは、六星占術の運命星理論において互いのエネルギー極性が自然に補完し合う好相性とされます。陽(+)の能動的な推進力と陰(-)の受容的な安定力が調和し、運命周期の浮沈を二人で乗り越えやすくなります。片方が弱る時期にもう片方が支える関係を意識的に築きましょう。")
