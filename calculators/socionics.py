...
def calculate(person_a: dict, person_b: dict) -> dict:
    ...
    if relation_key == "dual":
        advice = "ソシオニクスの双対関係は、8つの情報要素が完全に補完し合う最高の組み合わせです。互いの弱い機能を相手が自然にカバーするため、一緒にいるだけで心理的な充足感が得られます。この稀有な関係を大切に育んでください。"
    elif relation_key == "activation":
        advice = "活性化関係では互いの主導機能と創造機能が刺激し合い、共にいると自然とエネルギーが湧きます。ソシオニクス理論上、短期的な相性は最高クラスです。長期関係では適度な距離感を保ち、互いの個人時間も尊重しましょう。"
    elif relation_key == "mirror":
        advice = "鏡像関係は同じ情報要素を扱いながらも、主導と創造の順序が逆転する知的パートナーシップです。互いの思考プロセスを映し出す鏡のような存在で、建設的な議論を通じて共に成長できます。意見の違いを対立ではなく補完と捉えましょう。"
    elif relation_key in ("conflict", "super_ego"):
        advice = "ソシオニクスでは情報代謝の型が衝突しやすい関係とされますが、これは互いに最も学びの多い組み合わせでもあります。相手の強い機能が自分の脆弱な機能を刺激するため、意識的に距離感を調整し、尊重の姿勢を保つことで大きな人間的成長が得られます。"
    elif relation_key in ("benefit", "beneficiary", "supervisor", "supervisee"):
        advice = "ソシオニクスの非対称関係では、一方が無意識に影響を与え他方が受ける構造が生じやすくなります。この力学を理解した上で、意識的に対等なコミュニケーションを心がけることが重要です。互いの貢献を言葉にして認め合う習慣を作りましょう。"
    else:
        advice_map = {
            5: "ソシオニクスの情報代謝理論において最高の適合性を示しています。互いの認知機能が自然に調和し、無理なく素晴らしい関係を築けるでしょう。",
            4: "ソシオニクス的にとても良い相性です。互いの情報処理スタイルが補完的に作用し、強みを最大限に活かし合える関係です。共同作業で特に力を発揮します。",
            3: "ソシオニクス的にバランスの取れた相性です。情報代謝の型に一定の違いがありますが、互いを理解しようとする努力が着実に実を結ぶ関係です。",
            2: "ソシオニクス的にはやや課題がある組み合わせですが、認知スタイルの違いが新たな視点をもたらし成長の糧になります。相手の情報処理の仕方を学ぶ姿勢が鍵です。",
            1: "ソシオニクス的には挑戦的な相性ですが、最も大きな学びと自己変革の機会を秘めた関係です。互いの違いを脅威ではなく成長の種と捉えてください。",
        }
