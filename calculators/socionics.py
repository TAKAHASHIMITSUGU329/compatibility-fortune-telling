...
def calculate(person_a: dict, person_b: dict) -> dict:
    ...
    if relation_key == "dual":
        advice = "ソシオニクスで最高の相性です！互いの弱点を自然に補い合える理想的なパートナーシップです。"
    elif relation_key == "activation":
        advice = "互いに活力を与え合える素晴らしい関係です。一緒にいると元気になれるでしょう。"
    elif relation_key == "mirror":
        advice = "似た価値観を持ちつつ、異なる視点を提供し合える関係です。互いから多くを学べます。"
    elif relation_key in ("conflict", "super_ego"):
        advice = "ソシオニクス的には課題の多い関係ですが、意識的に互いの違いを尊重することで成長できます。"
    elif relation_key in ("benefit", "beneficiary", "supervisor", "supervisee"):
        advice = "非対称な関係になりやすいので、互いの役割を意識し、バランスを保つことが大切です。"
    else:
        advice_map = {
            5: "ソシオニクス的に最高の相性です。自然体で素晴らしい関係を築けます。",
            4: "ソシオニクス的にとても良い相性です。互いの強みを活かし合えます。",
            3: "ソシオニクス的にバランスの取れた相性です。理解し合う努力が実を結びます。",
            2: "ソシオニクス的にはやや課題がありますが、違いが成長の糧になります。",
            1: "ソシオニクス的には挑戦的な相性ですが、大きな学びがあります。",
        }
