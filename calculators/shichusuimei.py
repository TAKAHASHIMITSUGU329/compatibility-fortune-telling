...
def calculate(person_a: dict, person_b: dict) -> dict:
    ...
    if "月柱完全一致" in special_relations:
        advice_parts.append("月柱の完全一致は非常に珍しく、価値観や感情面で深い共鳴がある証です。")
    if "日干合" in special_relations:
        advice_parts.append("日干合は最も強い縁を示し、運命的な出会いと言えます。")
    if relationship == "比和":
        advice_parts.append("同じ五行同士で自然と分かり合える関係です。")
    elif "相生" in relationship:
        advice_parts.append("相生の関係は互いを育て合い、成長を促します。")
    elif "相剋" in relationship:
        advice_parts.append("相剋の関係には課題もありますが、それが成長の原動力になります。")

    if not advice_parts:
        advice_map = {
            5: "四柱推命的に最高の相性です。天が結んだ縁を大切にしてください。",
            4: "四柱推命的にとても良い相性です。互いの運気を高め合える関係です。",
            3: "四柱推命的にバランスの取れた相性です。お互いを尊重することで良い関係が築けます。",
            2: "四柱推命的にやや課題のある相性ですが、乗り越えることで絆が深まります。",
            1: "四柱推命的に試練の多い相性ですが、大きな成長をもたらす関係です。",
        }
