...
def calculate(person_a: dict, person_b: dict) -> dict:
    ...
    advice_map = {
        5: "ライフパス数の組み合わせが示す通り、二人の魂の使命が深く調和しています。数秘術では人生の根本的な方向性が一致する稀有な関係とされ、共通の目標に向かって自然に協力し合えるでしょう。",
        4: "ライフパス数の相性が高く、互いの持つ数のエネルギーが補完関係にあります。数秘術の理論では異なる数の振動が共鳴することで新たな可能性が開くとされており、お互いの強みを意識的に活かし合うことで関係がさらに発展します。",
        3: "ライフパス数の相性は標準的で、誕生数との複合分析では潜在的な共鳴が見られます。数秘術では数の波動の違いが学びをもたらすとされるため、相手の価値観を理解する努力が実を結ぶでしょう。",
        2: "ライフパス数の組み合わせにはやや課題がありますが、数秘術ではこの緊張関係こそが魂の成長を促すとされています。互いの異なる数のエネルギーを尊重し、違いを強みとして捉えることで関係は深まります。",
        1: "ライフパス数が示す人生の方向性に大きな違いがありますが、数秘術ではこの挑戦的な組み合わせが最も深い変容をもたらすとされています。困難を共に乗り越える過程で、かけがえのない絆が育まれるでしょう。",
    }
